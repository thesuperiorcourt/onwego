...
| T5 | Win tiers (min / full / boss) | Three buttons call `logTask()` with a payout tier; each grants XP, currency, a loot chance, and plants something in the Garden | Three ways to succeed so a bad session still counts | Tasks, tracks, XP, currency, loot, garden, streak | ok |
...
| T18 | Missed section | A default section (`scope:'missed'`, built on the same `isMissed()` check Timeline and Pace use) listing every unresolved missed task, positioned ahead of Up next. Each card carries the same three resolutions as the Pace sheet (`data-missed`: move to today, fold in, let it go) directly inline, via `isMissed()` in `taskCard()` — no detour through Timeline or Pace needed | Falling behind should be visible, and resolvable, on the screen you actually open, not only on Timeline | Tasks, sections config, K1 | ok — inline resolution added so the decision doesn't require leaving Today; existing worlds with an untouched default layout pick up the Missed section automatically, customised layouts don't |

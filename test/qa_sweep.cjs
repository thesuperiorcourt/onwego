...
setTimeout(()=>{
  ...
  // editors
  [['task editor',"openSheet(taskEditor(W.tasks[0].id))"],
   ['new task',"openSheet(taskEditor('new'))"],
   ['track editor',"openSheet(trackEditor(W.tracks[0].id))"],
   ['new track',"openSheet(trackEditor('new'))"],
   ['section editor',"openSheet(sectionEditor(W.home.sections[0].id))"]].forEach(([name,code])=>{

...
- **RLS isn't armed — half done.** `DATABASE_URL` still connects as `neondb_owner` (`rolbypassrls = t`), so the policies in `db/schema.sql` still aren't enforced; isolation currently rests entirely on the function filtering by the verified `sub`. The code side is done and live: `netlify/functions/sync.mjs` now scopes every query inside a transaction that sets `app.user_id` first, via a `scoped()` helper — harmless today, since RLS is bypassed either way, but it's what the policies need to actually apply. What's left is entirely database-side: `db/rls_role.sql` creates a restricted `onwego_api` role (fresh role, `rolbypassrls` false by default) with just the grants the function needs — run it by hand in the Neon Console (needs a real password swapped in for the placeholder first), then generate a connection string for that role from Neon Console → Connect, set it as `DATABASE_URL` in Netlify, and verify sync still works end-to-end before considering this done. This step got blocked on tooling permissions (no way to run DDL against the production database or set Netlify env vars from this session) — needs a human either running `db/rls_role.sql` directly or granting that capability.
